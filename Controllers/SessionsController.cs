...
public class SessionsController : ControllerBase
{
    ...
    public async Task<ActionResult<List<KAgentSession>>> GetAll()
    {
        try
        {
            return await _kagent.GetSessionsAsync();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to get sessions");
            return new List<KAgentSession>();
        }
    }
    ...
    public async Task<ActionResult<KAgentSession>> Create([FromBody] CreateSessionRequest request)
    {
        ...
        {
            _logger.LogError(ex, "Failed to create session for agent {Agent}", request.AgentName);
    ...
    public async Task<ActionResult> Delete(string id)
    {
        try
        {
            await _kagent.DeleteSessionAsync(id);
            return NoContent();
        }
        catch (Exception ex)
        {
    ...
    public async Task<ActionResult<List<ChatMessage>>> GetMessages(string id)
    {
        try
        {
            return await _kagent.GetSessionMessagesAsync(id);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to get messages for session {Id}", id);
    ...
    public async Task<ActionResult<List<KAgentEvent>>> GetEvents(string id, [FromQuery] int? limit = null)
    {
        try
        {
            return await _kagent.GetSessionEventsAsync(id, limit);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to get events for session {Id}", id);
