...
public class KAgentChatService : IChatService
{
    ...
    public async Task<ChatResponse> SendMessageAsync(ChatRequest request)
    {
        ...
        catch (Exception ex) { _logger.LogError(ex, "Failed to send message to agent {Agent}", request.AgentName); throw; }
    }
    ...
    public async Task<List<ChatMessage>> GetConversationAsync(string conversationId)
    {
        ...
        catch (Exception ex) { _logger.LogError(ex, "Failed to get conversation {Id}", conversationId); return new(); }
    }
