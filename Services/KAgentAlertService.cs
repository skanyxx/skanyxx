...
public class KAgentAlertService : IAlertService
{
    ...
    public async Task<List<Alert>> GetAllAsync(string? severity = null, string? status = null)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(severity)) alerts = alerts.Where(a => a.Severity == severity).ToList();
            if (!string.IsNullOrEmpty(status))   alerts = alerts.Where(a => a.Status   == status).ToList();
            return alerts;
        }
        catch (Exception ex) { _logger.LogError(ex, "Failed to get alerts"); return new(); }
    }
